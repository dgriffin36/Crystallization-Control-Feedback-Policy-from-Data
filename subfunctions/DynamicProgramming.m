function [Policy]=DynamicProgramming(T,sTar,rho,gamma,NS,cGrid,mGrid,sGrid)
...
for i=1:nm
    for j=1:nc
        V((j-1)*nm+i,T)=1*norm([(cGrid(j)-sTar(1))/25;mGrid(i)-sTar(2)])^2;
    end
end
...
while t>=1
    for i=1:nm
        for j=1:nc
            [V((j-1)*nm+i,t),A((j-1)*nm+i,t)]=CalcMinVal([cGrid(j);mGrid(i)],i,j,sTar,V(:,t+1),t,T,rho,gamma,NS,sGrid,mGrid);
